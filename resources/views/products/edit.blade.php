<x-company-panel-layout :company="$company">
    <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight mb-6">
        Editando Producto: {{ $product->description }}
    </h2>
        ...
        <div class="p-6 md:p-8 text-gray-900 dark:text-gray-100">
            ...
            <form method="POST" action="{{ route('products.update', $product) }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input id="description" name="description" type="text" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:bg-gray-900 dark:border-gray-600" value="{{ old('description', $product->description) }}" required />
                    </div>

                    <!-- SKU -->
                    <div>
                        <label for="sku" class="block font-medium text-sm text-gray-700 dark:text-gray-300">SKU / Núm. de Parte</label>
                        <input id="sku" name="sku" type="text" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:bg-gray-900 dark:border-gray-600" value="{{ old('sku', $product->sku) }}" />
                    </div>

                    <!-- Precio Unitario -->
                    <div>
                        <label for="price" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Precio Unitario (sin IVA)*</label>
                        <input id="price" name="price" type="number" step="0.01" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:bg-gray-900 dark:border-gray-600" value="{{ old('price', $product->price) }}" required />
                    </div>

                    <!-- Buscador Inteligente para Clave de Producto/Servicio -->
                    <div x-data="autocomplete('{{ route('api.sat_product_keys.search') }}', '{{ $product->sat_product_key }}', '{{ $satProductKeyName }}')" class="relative">
                    ...
                    <div>
                        ...
                        <select id="sat_unit_key" name="sat_unit_key" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:bg-gray-900 dark:border-gray-600" required>
                            @foreach ($satUnitKeys as $code => $name)
                                <option value="{{ $code }}" @selected(old('sat_unit_key', $product->sat_unit_key) == $code)>{{ $code }} - {{ $name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-2">
                         <label for="taxes" class="inline-flex items-center">
                            <input type="checkbox" id="taxes" name="taxes" value="1" @checked(old('taxes', $product->taxes)) class="rounded dark:bg-gray-900 border-gray-300 dark:border-gray-700 text-indigo-600 shadow-sm">
            ...
            <select id="student_id" name="student_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:bg-gray-900 dark:border-gray-600">
                ...
                @foreach ($students as $student)
                    <option value="{{ $student->id }}" @selected(old('student_id', $product->student_id) == $student->id)>{{ $student->name }} ({{ $student->curp }})</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <script>
        function autocomplete(apiUrl, initialCode = '', initialName = '') {
            return {
                search: initialCode ? `${initialCode} - ${initialName}` : '',
                results: [],
                showResults: false,
