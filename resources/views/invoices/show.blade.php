            ...
            <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-200 flex items-center gap-3">
                ...
                @php
                    $colorClass = match($invoice->status) {
                        'draft' => 'bg-gray-100 text-gray-800 border-gray-300 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600',
                        'issued' => 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300',
                        'cancelled' => 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300',
                        default => 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300'
                    };
                @endphp
                <span class="px-3 py-1 text-sm rounded-full font-semibold border {{ $colorClass }}">
                    {{ $invoice->status_es }}
                </span>
            </h1>
            @if($invoice->uuid)
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">UUID: {{ $invoice->uuid }}</p>
            @endif
        </div>

        <div class="flex items-center space-x-3">
            <a href="{{ route('invoices.index', ['company_id' => $invoice->company_id]) }}" class="px-4 py-2 bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-gray-200 rounded-md hover:bg-gray-300 dark:hover:bg-gray-600 text-sm font-semibold transition">
                ← Volver
            </a>
            
            @if($invoice->status === 'issued')
                <a href="{{ route('invoices.pdf', $invoice) }}" class="px-4 py-2 bg-indigo-50 text-indigo-700 border border-indigo-200 dark:bg-indigo-900/50 dark:text-indigo-300 dark:border-indigo-700 rounded-md hover:bg-indigo-100 dark:hover:bg-indigo-800 text-sm font-bold transition">
                    📄 Descargar PDF
                </a>
                <a href="{{ route('invoices.xml', $invoice) }}" class="px-4 py-2 bg-indigo-50 text-indigo-700 border border-indigo-200 dark:bg-indigo-900/50 dark:text-indigo-300 dark:border-indigo-700 rounded-md hover:bg-indigo-100 dark:hover:bg-indigo-800 text-sm font-bold transition">
                    💻 Descargar XML
                </a>
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
                ...
                @elseif($invoice->payment_method === 'PUE')
                    <div class="text-center py-6 text-green-500 dark:text-green-400">
                        <p class="text-4xl">✅</p>
                ...
                @elseif($invoice->payment_method === 'PPD')
                    ...
                    <div class="space-y-4">
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-600 dark:text-gray-300">Cobrado: <strong>${{ number_format($totalPaid, 2) }}</strong></span>
                        ...
                        @if($isFullyPaid)
                            <div class="p-3 bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 text-green-700 dark:text-green-400 rounded text-center text-sm font-bold">
                                🎉 ¡Factura liquidada!
                            </div>
                        @else
                            <a href="{{ route('payments.index', $invoice) }}" class="block w-full text-center px-4 py-2 bg-blue-600 dark:bg-blue-700 text-white rounded-md hover:bg-blue-700 dark:hover:bg-blue-600 text-sm font-bold transition shadow">
                                💳 Registrar Pago (REP)
                            </a>
                        @endif
                    </div>
                    ...
                    <div class="mt-8">
                        ...
                        @if($invoice->payments->where('status', '!=', 'cancelled')->isEmpty())
                            <p class="text-sm text-gray-500 dark:text-gray-500 italic text-center">Aún no se han registrado pagos.</p>
                        @else
                            <div class="relative border-l-2 border-gray-200 dark:border-gray-700 ml-3 space-y-4">
                                @foreach($invoice->payments->where('status', '!=', 'cancelled') as $payment)
                                    <div class="relative pl-6">
                                        ...
                                        <p class="text-xs text-gray-500 dark:text-gray-400">Fecha: {{ \Carbon\Carbon::parse($payment->payment_date)->format('d/m/Y') }}</p>
                                        <p class="text-xs text-gray-400 dark:text-gray-500">UUID: {{ substr($payment->uuid, 0, 13) }}...</p>
                                    </div>
                                @endforeach
                            </div>
                        @endif
