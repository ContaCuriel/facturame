    ...
    @method('PUT')
                    ...
                    <div class="flex items-center justify-end mt-8 space-x-4">
                        ...
                        <button type="submit" name="action" value="draft"
                            class="inline-flex items-center px-4 py-2 bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-gray-800 uppercase tracking-widest hover:bg-gray-300 transition ease-in-out duration-150 disabled:opacity-50"
                            :disabled="!selectedClient.id || items.length === 0">
                            💾 Guardar Borrador
                        </button>

                        <!-- Botón 2: Vista Previa PDF (El atributo formtarget="_blank" abre la pestaña nueva) -->
                        <button type="submit" name="action" value="preview" formtarget="_blank"
                            class="inline-flex items-center px-4 py-2 bg-blue-100 border border-transparent rounded-md font-semibold text-xs text-blue-700 uppercase tracking-widest hover:bg-blue-200 transition ease-in-out duration-150 disabled:opacity-50"
                            :disabled="!selectedClient.id || items.length === 0">
                            👁️ Vista Previa PDF
                        </button>

                        <!-- Botón 3: Timbrar Factura -->
                        <button type="submit" name="action" value="issue"
                            class="inline-flex items-center px-6 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-500 transition ease-in-out duration-150 disabled:opacity-50 shadow-md"
                            :disabled="!selectedClient.id || items.length === 0">
                            🚀 Timbrar Factura
                        </button>
